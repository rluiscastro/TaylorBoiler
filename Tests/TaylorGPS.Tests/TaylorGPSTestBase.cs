...
using TaylorGPS.Migrations.SeedData;
...
namespace TaylorGPS.Tests
{
    public abstract class TaylorGPSTestBase : AbpIntegratedTestBase<TaylorGPSTestModule>
    {
        ...
        protected void UsingDbContext(Action<TaylorGPSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TaylorGPSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TaylorGPSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TaylorGPSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TaylorGPSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaylorGPSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TaylorGPSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TaylorGPSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TaylorGPSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TaylorGPSDbContext, Task<T>> func)
        {
            T result;
